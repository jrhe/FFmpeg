...
fn parse_marker(line: &[u8]) -> Option<i64> {
    let s = skip_ws(line);
    if !s.starts_with(b"-->>") {
        return None;
    }
...
#[no_mangle]
pub extern "C" fn ffmpeg_rs_aqtitle_parse_marker(
    line: *const u8,
    line_len: usize,
    out_frame: *mut i64,
) -> c_int {
    if line.is_null() || out_frame.is_null() {
        return -1;
    }
    let buf = unsafe { core::slice::from_raw_parts(line, line_len) };
    let v = match parse_marker(buf) {
        Some(v) => v,
        None => return -2,
    };
    unsafe {
...
mod tests {
    ...
    #[test]
    fn parses_marker() {
        assert_eq!(parse_marker(b"-->> 123"), Some(123));
    }
}
