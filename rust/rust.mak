...
RUST_FFMPEG_MCC_DIR := $(SRC_PATH)/rust/ffmpeg-mcc
RUST_FFMPEG_MCC_LIB := $(RUST_FFMPEG_MCC_DIR)/target/$(if $(RUST_TARGET),$(RUST_TARGET),)/$(RUST_ARTIFACT_SUBDIR)/libffmpeg_mcc.a

$(RUST_FFMPEG_MCC_LIB):
	$(M)cd $(RUST_FFMPEG_MCC_DIR) && $(CARGO) -q build $(RUST_CARGO_PROFILE) $(RUST_CARGO_TARGET_ARG)

clean::
	$(Q)cd $(RUST_FFMPEG_MCC_DIR) && $(CARGO) -q clean || true

RUST_FFMPEG_ID3V2_DIR := $(SRC_PATH)/rust/ffmpeg-id3v2
RUST_FFMPEG_ID3V2_LIB := $(RUST_FFMPEG_ID3V2_DIR)/target/$(if $(RUST_TARGET),$(RUST_TARGET),)/$(RUST_ARTIFACT_SUBDIR)/libffmpeg_id3v2.a

$(RUST_FFMPEG_ID3V2_LIB):
	$(M)cd $(RUST_FFMPEG_ID3V2_DIR) && $(CARGO) -q build $(RUST_CARGO_PROFILE) $(RUST_CARGO_TARGET_ARG)

clean::
	$(Q)cd $(RUST_FFMPEG_ID3V2_DIR) && $(CARGO) -q clean || true

RUST_FFMPEG_CONCAT_DIR := $(SRC_PATH)/rust/ffmpeg-concat
RUST_FFMPEG_CONCAT_LIB := $(RUST_FFMPEG_CONCAT_DIR)/target/$(if $(RUST_TARGET),$(RUST_TARGET),)/$(RUST_ARTIFACT_SUBDIR)/libffmpeg_concat.a

$(RUST_FFMPEG_CONCAT_LIB):
	$(M)cd $(RUST_FFMPEG_CONCAT_DIR) && $(CARGO) -q build $(RUST_CARGO_PROFILE) $(RUST_CARGO_TARGET_ARG)

clean::
	$(Q)cd $(RUST_FFMPEG_CONCAT_DIR) && $(CARGO) -q clean || true

RUST_FFMPEG_DATA_URI_DIR := $(SRC_PATH)/rust/ffmpeg-data-uri
RUST_FFMPEG_DATA_URI_LIB := $(RUST_FFMPEG_DATA_URI_DIR)/target/$(if $(RUST_TARGET),$(RUST_TARGET),)/$(RUST_ARTIFACT_SUBDIR)/libffmpeg_data_uri.a

$(RUST_FFMPEG_DATA_URI_LIB):
	$(M)cd $(RUST_FFMPEG_DATA_URI_DIR) && $(CARGO) -q build $(RUST_CARGO_PROFILE) $(RUST_CARGO_TARGET_ARG)

clean::
	$(Q)cd $(RUST_FFMPEG_DATA_URI_DIR) && $(CARGO) -q clean || true

RUST_FFMPEG_FFMETADATA_DIR := $(SRC_PATH)/rust/ffmpeg-ffmetadata
RUST_FFMPEG_FFMETADATA_LIB := $(RUST_FFMPEG_FFMETADATA_DIR)/target/$(if $(RUST_TARGET),$(RUST_TARGET),)/$(RUST_ARTIFACT_SUBDIR)/libffmpeg_ffmetadata.a

$(RUST_FFMPEG_FFMETADATA_LIB):
	$(M)cd $(RUST_FFMPEG_FFMETADATA_DIR) && $(CARGO) -q build $(RUST_CARGO_PROFILE) $(RUST_CARGO_TARGET_ARG)

clean::
	$(Q)cd $(RUST_FFMPEG_FFMETADATA_DIR) && $(CARGO) -q clean || true

RUST_FFMPEG_UTIL_PARSE_DIR := $(SRC_PATH)/rust/ffmpeg-util-parse
RUST_FFMPEG_UTIL_PARSE_LIB := $(RUST_FFMPEG_UTIL_PARSE_DIR)/target/$(if $(RUST_TARGET),$(RUST_TARGET),)/$(RUST_ARTIFACT_SUBDIR)/libffmpeg_util_parse.a

$(RUST_FFMPEG_UTIL_PARSE_LIB):
	$(M)cd $(RUST_FFMPEG_UTIL_PARSE_DIR) && $(CARGO) -q build $(RUST_CARGO_PROFILE) $(RUST_CARGO_TARGET_ARG)

clean::
	$(Q)cd $(RUST_FFMPEG_UTIL_PARSE_DIR) && $(CARGO) -q clean || true

# Build all known Rust crates.
rust-libs: $(RUST_FFMPEG_FFI_LIB) $(RUST_FFMPEG_HLSWRITER_LIB) $(RUST_FFMPEG_HLSPARSER_LIB) $(RUST_FFMPEG_WEBVTT_LIB) $(RUST_FFMPEG_SUBRIP_LIB) $(RUST_FFMPEG_MICRODVD_LIB) $(RUST_FFMPEG_TTML_LIB) $(RUST_FFMPEG_MPL2_LIB) $(RUST_FFMPEG_VPLAYER_LIB) $(RUST_FFMPEG_JACOSUB_LIB) $(RUST_FFMPEG_SUBVIEWER_LIB) $(RUST_FFMPEG_SCC_LIB) $(RUST_FFMPEG_STL_LIB) $(RUST_FFMPEG_LRC_LIB) $(RUST_FFMPEG_MPSUB_LIB) $(RUST_FFMPEG_PJS_LIB) $(RUST_FFMPEG_REALTEXT_LIB) $(RUST_FFMPEG_AQTITLE_LIB) $(RUST_FFMPEG_SAMI_LIB) $(RUST_FFMPEG_ASS_LIB) $(RUST_FFMPEG_MCC_LIB) $(RUST_FFMPEG_ID3V2_LIB) $(RUST_FFMPEG_CONCAT_LIB) $(RUST_FFMPEG_DATA_URI_LIB) $(RUST_FFMPEG_FFMETADATA_LIB) $(RUST_FFMPEG_UTIL_PARSE_LIB)
